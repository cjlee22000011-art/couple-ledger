    ...
    <div className="space-y-6">
      <div className="card p-4">
        ...
        <form onSubmit={saveName} className="flex gap-2">
          ...
          />
          <button className="bg-ledger text-white rounded px-4 font-bold hover:bg-ledger-light">
            保存
          </button>
        </form>
      ...
      <div className="card p-4">
        ...
        <form onSubmit={addCategory} className="flex flex-wrap gap-2 mb-4">
          <input
            ...
            className="w-14 shrink-0 border border-line rounded px-2 py-2 text-center bg-white"
            maxLength={2}
          />
          <input
            value={newCatName}
            ...
            placeholder="分类名称"
            className="flex-1 min-w-[140px] border border-line rounded px-3 py-2 bg-white"
          />
          <select
            value={newCatType}
            onChange={(e) => setNewCatType(e.target.value as TxType)}
            className="border border-line rounded px-2 py-2 bg-white shrink-0"
          >
