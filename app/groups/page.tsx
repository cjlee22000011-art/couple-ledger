...
function GroupsPageInner() {
  ...
  // 支持"分享邀请链接"进来后自动带上 ?join=邀请码，登录后自动尝试加入一次
  useEffect(() => {
    const code = searchParams.get("join");
    if (code && session && !autoJoinedRef.current) {
      autoJoinedRef.current = true;
      setJoinCode(code);
      joinByCode(code);
    }
  }, [searchParams, session, joinByCode]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card divide-y divide-line">
        <h2 className="ledger-stamp font-bold text-ledger p-4 pb-0">
          我的群组
        </h2>
        {groups.length === 0 && (
